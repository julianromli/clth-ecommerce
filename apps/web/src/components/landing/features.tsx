...
export default function Features() {
  return (
    <section className="w-full border-b border-gray-100 bg-white px-6 py-8 md:px-12 md:py-16">
      <div className="mx-auto grid max-w-[1200px] grid-cols-1 gap-8 md:grid-cols-2 md:gap-4 lg:grid-cols-4">
        {features.map((feature) => (
          <div
            key={feature.title}
            className="flex items-center justify-center space-x-4 md:justify-start"
          >
            <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full border border-gray-200 text-gray-800">
              {feature.icon}
            </div>
            <div>
