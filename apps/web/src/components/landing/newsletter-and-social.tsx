  ...
  return (
    ...
    >
      <div className="flex flex-col justify-between gap-16 lg:flex-row lg:gap-8">
        <div className="flex w-full flex-col justify-center lg:w-[45%]">
          ...
          >
            <input
              ...
              className="flex-grow rounded-lg border border-gray-200 bg-gray-50 px-5 py-3.5 text-[15px] transition-colors placeholder:text-gray-400 focus:border-brand-dark focus:ring-1 focus:ring-brand-dark focus:outline-none"
              required
            />
            <button
              type="submit"
              className="whitespace-nowrap rounded-lg bg-brand-dark px-8 py-3.5 text-[15px] font-medium text-white transition-colors hover:bg-[#1E2721]"
            >
              Subscribe
            </button>
          </form>
        ...
        <div className="flex w-full gap-4 md:gap-6 lg:w-[55%]">
          {socialImages.map((imgUrl, index) => (
            <a
              key={imgUrl}
              href="#journal"
              className="group relative block aspect-square flex-1 overflow-hidden rounded-xl bg-gray-100 md:aspect-[4/5]"
            >
              <img
                src={imgUrl}
